    ...
    private void addNotifyToView(final NiftyNotificationView notify) {
        ...
        if (null == notifyView.getParent()) {
            ...
            if (null == params) {
                ...
                ((ViewGroup.MarginLayoutParams) params).setMargins(0, 40, 0, 0);
            }
            ViewGroup viewGroup = notify.getViewGroup();
            if (null != viewGroup) {
                if (viewGroup instanceof FrameLayout) {
                    viewGroup.addView(notifyView, params);
                } else {
                    viewGroup.addView(notifyView, 0, params);
                }
        ...
        if (null != observer) {
            observer.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    ...
                    if (! isSticky) {
                        long displayDuration = notify.getDispalyDuration();
                        if (displayDuration != NiftyNotificationView.DISPLAY_ALL_TIME)
                            sendMessageDelayed(notify, Messages.REMOVE_NOTIFICATION,
                                    displayDuration
